...
│   │   └── updaters/         # uma estratégia (Strategy) por tipo de item + registry injetável
│   ├── application/
│   │   └── gilded_rose.py    # orquestração, recebe o registry por injeção e delega
...
2. Substituir os condicionais por uma estratégia (Strategy/polimorfismo) isolada por tipo de item, em `src/domain/updaters/`, conectada por um `UpdaterRegistry` injetado em `GildedRose` (Dependency Inversion).
